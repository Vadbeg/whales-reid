...
if __name__ == '__main__':
    ...
    DATA_CONFIG_PATH = 'configs/data_118_server_fins.yaml'
    CHECKPOINT_PATH = (
        '/home/vadim-tsitko/Projects/SERVER/whales-reid/logs/'
        'HappyWhale/muvob1p5/checkpoints/epoch=8-step=4157_copy.ckpt'
    )

    _data_config = load_yaml(yaml_path=DATA_CONFIG_PATH)
    _model = EfficientNetModel(
        model_type='tf_efficientnet_b6', with_embeddings_layer=True, in_channels=3
    )
    ...
    _pred_dataframe.to_csv(path_or_buf='results/submission_part.csv', index=False)
